...
def create(creature: Creature) -> Creature:
    ...
    if curs:
        try:
            ...
            log.error("IntegrityError: cannot create creature %s: %s", creature.name, e)
            raise DuplicateError(f"Duplicate creature {creature.name}") from e
    else:
    ...
    if curs:
        try:
            ...
            raise DuplicateError(
                f"Duplicate creature name: {creature.name}"
            ) from e
        if curs.rowcount == 1:
            return get_one(creature.name)
        else:
