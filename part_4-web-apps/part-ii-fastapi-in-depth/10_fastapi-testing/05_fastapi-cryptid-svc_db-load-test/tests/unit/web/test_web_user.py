...
from cryptid.fake.user import _reset, model_to_dict
from cryptid.model.user import User
from cryptid.web import user
...
def test_modify(fakes):
    got = user.modify(
        fakes[0].name,
        {"name": "mod_username", "password_hash": "mod_password_hash"},
    )
...
def test_modify_missing(sample: User):
    with pytest.raises(HTTPException) as e:
        user.modify(sample.name, model_to_dict(sample))
