...
> pathlib, Path, rglob, pytest, monkeypatch, mock factory functions
...
HINT: the program should start by traversing the directory tree creating a dictionary by file sizes, as it is required for two files to be the same to have the same size. Then, for the list of potential candidates, a more robust strategy can be used to identify the real duplicates amongst them.
