...
+ [FastAPI basic concepts](#fastapi-basic-concepts)
    ...
    + [Middleware](#middleware)
        ...
        + [Execution order when having multiple middlewares](#execution-order-when-having-multiple-middlewares)
    + [CORS (Cross-Origin Resource Sharing)](#cors-cross-origin-resource-sharing)
        + [Origin](#origin)
        + [Using `CORSMiddleware`](#using-corsmiddleware)
        + [CORS preflight requests](#cors-preflight-requests)
        + [Simple requests](#simple-requests)
    + [Using SQLModel in FastAPI](#using-sqlmodel-in-fastapi)
        + [Creating an app with a single model](#creating-an-app-with-a-single-model)
            + [Creating the model](#creating-the-model)
            + [Creating an engine](#creating-an-engine)
            + [Creating the tables](#creating-the-tables)
            + [Creating a Session dependency](#creating-a-session-dependency)
            + [Creating the DB tables on startup](#creating-the-db-tables-on-startup)
            + [Path operation for creating a model in the DB](#path-operation-for-creating-a-model-in-the-db)
            + [Path operation for reading models from the DB](#path-operation-for-reading-models-from-the-db)
            + [Path operation for reading one model by id](#path-operation-for-reading-one-model-by-id)
            + [Path operation for deleting a model by id](#path-operation-for-deleting-a-model-by-id)
        + [Creating an app with multiple models](#creating-an-app-with-multiple-models)
            + [The app with multiple models](#the-app-with-multiple-models)
            + [Creating with `HeroCreate` and returning a `HeroPublic`](#creating-with-herocreate-and-returning-a-heropublic)
            + [Updating a hero with `HeroUpdate`](#updating-a-hero-with-heroupdate)
    + [Structuring bigger applications](#structuring-bigger-applications)
        + [An example](#an-example)
        + [APIRouter](#apirouter)
        + [Dependencies](#dependencies-1)
        + [More modules with APIRouter](#more-modules-with-apirouter)
        + [The main `FastAPI`](#the-main-fastapi)
        + [Including the same router multiple times with different prefix](#including-the-same-router-multiple-times-with-different-prefix)
        + [Including an APIRouter in another](#including-an-apirouter-in-another)
    ...
    + [Metadata and Docs URLs](#metadata-and-docs-urls)
        + [Metadata for APIs](#metadata-for-apis)
            + [License identifier](#license-identifier)
        + [Creating metadata for tags](#creating-metadata-for-tags)
...
Therefore, the lifecycle of a FastAPI app with middleware is:

1. Middleware takes each request that comes to your app.
1. Middleware performs some logic, potentially reading/updating that request.
1. FastAPI then passes the request to be processed to the corresponding path operation.
1. Middleware receives the response generated by the path operation.
1. Middleware performs some logic, potentially reading/updating that reponse.
1. FastAPI returns the response.
...
+ the `Request` object.
+ a `call_next` parameter, which is a function the receive the `Request` as a parameter and will:
    + Pass the `request` to the corresponding path operation.
    + Return the `response` generated by the path operation.

Therefore, after invoking `call_next()` you can act on the `response`:
...
#### Execution order when having multiple middlewares

When you add multiple middlewares using either `@app.middleware()` decorator or `app.add_middleware()`, each new middleware wraps the application, forming a stack. The last middleware added is the outermost, and the first is the innermost, so that if you have:
...
```
Request -> MiddlewareB -> MiddlewareA -> path operation ->
-> MiddlewareA -> MiddlewareB -> Response
```

### CORS (Cross-Origin Resource Sharing)

CORS or "Cross-Origin Resource Sharing" refers to situations when a frontend running in a browser has JavaScript code that communicates with a backend that sits in a different "origin" than the frontend.

Let's say you have a frontend running in your browser at http://localhost:8080, and its JavaScript is trying to send requests to a backend hosted on http://localhost (by default http will assume the port is 80).

The browser will send an HTTP `OPTIONS` request to localhost:80, and if the backend sends the appropriate headers authorizing the communication from this different origin (http://localhost:8080 is the origin trying to communicate with http://localhost), then the browser will allow the JavaScript in the frontend send the request.

To achieve this, the backed host in localhost:80 must send back a list of *allowed origins*.

#### Origin

An origin is the combination of:
+ the protocol used: http or https
+ the domain: myapp.com, localhost, localhost.myapp.com, ...
+ the port: 80, 443, 5000, ...

Note that it's also possible to declare the list as a `"*"` (a wildcard) to say that all are allowed in certain types of communications, but a wildcard won't work on anything that involves credentials.

#### Using `CORSMiddleware`

FastAPI provides the `CORSMiddleware` to create a list of origins and specify whether your backend allows credentials, the methods allowed, and the specific HTTP headers allowed.
...
origins = [
    "http://localhost.myapp.com",
    "https://localhost.myapp.com",
    "http://localhost",
    "http://localhost:8080"
]
...
+ `allow_origins`: list of origins that should be permitted to make cross-origin requests. You can use ["*"] to allow all origins.
+ `allow_origin_regex`: regex string to match against origins that should be permitted to make cross-origin requests (e.g., `"https://.*\.example\.org"`)
+ `allow_methods`: a list of HTTP methods that should be allowed for cross-origin requests. Defaults to `["GET"]`. You can use `["*"]` to allow all standard methods.
+ `allow_headers`: a list of HTTP request headers that should be supported for cross origin requests. Defaults to `[]`. You can use `["*"]` to allow all headers. Note that the following ones are always allowed for simple CORS requests:
    + `Accept`
    + `Accept-Language`
    + `Content-Language`
    + `Content-Type`
...
+ `expose_headers`: indicate that response headers should be made available to the browser. Defaults to `[]`.
+ `max_age`: sets a maximum time in seconds for browsers to cache CORS responses. Defaults to 600.

#### CORS preflight requests

These are `OPTIONS` request with `Origin` and `Access-Control-Request-Method` headers. In this case, the middleware will intercept the incoming request, and respond with appropriate CORS headers and either a `200` or `400` response for informational purposes.

#### Simple requests

Any request with an `Origin` header will make the middleware to include the appropriate CORS headers on the response.

### Using [SQLModel](https://sqlmodel.tiangolo.com/) in FastAPI

[SQLModel](https://sqlmodel.tiangolo.com/) is a package built on top of [SQLAlchemy](https://www.sqlalchemy.org/) and Pydantic and it's a good solution if you need to use SQL databases in your FastAPI application.


When using SQLModel you can target the following DB engines:
+ PostgreSQL
+ MySQL
+ SQLite
...
#### Creating an app with a single model

The following snippet illustrates a FastAPI app using SQLModel declaring a single database model `Hero`:
...
The model is created like a regular Pydantic model:
...
| NOTE: |
| :---- |
| Behind the scenes, `Hero` is actually a Pydantic model. |

The following aspects must be noted:
+ `table=True` tells SQLModel that this is a table model (repesent a table in the SQL database), and not just a data model (as would be the case for other Pydantic models).
+ `Field(primary_key=True)` tells SQLModel that `id` is the primary key in the SQL database.

    `id` is defined as an `int | None` so that you can create an object without an `id`, assuming the DB will generate it when saving. SQLModel understands that the DB will provide the `id` and defines the column as a non-null INTEGER in the schema.
+ `Field(index=True)` tells SQLModel that it should create a SQL index for this column to enable faster lookups in the DB when reading data filtered by this column.

##### Creating an engine

A SQLModel engine is the object that holds the connections to the db.

You need to have a single `engine` object for all the code in your app that interacts with the same DB.
...
Using `check_same_thread=False` allows FastAPI to use the same SQLite database in different threads. This is necessary as one single request could use more than one thread.

##### Creating the tables

The function `SQLModel.metadata.create_all(engine)` lets you create the tables associated to all the table models:
...
You can create a FastAPI dependency with `yield` providing a new `Session` (but a single one) per request:
...
SessionDep = Annotated[Session, Depends(get_session)] # type alias
```

| NOTE: |
| :---- |
| Implementing `get_session()` with `yield` ensures that a single session per request is created and used. |

##### Creating the DB tables on startup
...
| NOTE: |
| :---- |
| In production, you may want to rely on [Alembic](https://alembic.sqlalchemy.org/en/latest/). SQLModel migration utilities are in the roadmap. |

##### Path operation for creating a model in the DB

Because SQLModel is a Pydantic model, you can use it with the same type annotations you'd use a Pydantic model.

Because SQLModel is not async, you must declare the path operation as a regular function and not as a coroutine.
...
def create_hero(hero: Hero, session: SessionDep) -> Hero:
    session.add(hero)
    session.commit()
    session.refresh(hero) # so that `id` is populated
    return hero
```

##### Path operation for reading models from the DB

We can read `Hero` instances from the DB using `select()`. We can include `limit()` and `offset()` for the server-side pagination of results.
...
##### Path operation for reading one model by id

You can read the details of a single `Hero` using `session.get()`:
...
##### Path operation for deleting a model by id

You can delete a `Hero` by bringing a `Hero` instance into the session and then using `session.delete()`:
...
#### Creating an app with multiple models

When using SQLModel, any model class featuring `table=True` is a table model. Any model class without `table=True` is just a Pydantic model with a few minor extra features.

In any case, you can use inheritance in SQLModel to avoid duplication.

For example, you can create a `HeroBase(SQLModel)`, as the base class featuring the fields used in all the models:
...
Then you can create `Hero`, which is the actual table model, with the extra fields needed:
...
You can also create a `HeroPublic` model to be used by the clients of the API. This will feature the same fields as `HeroBase`, so the `secret_name` won't be shared with clients.
...
Then, we can create a `HeroCreate` model, which is the model we'll use to validate the data sent from the client. This one should feature the `secret_name` and no `id`:
...
Along the same lines, we can create a `HeroUpdate` to support the updates. In terms of fields, this will have the same fields needed to create a new hero (`HeroCreate`), but all the fields are optional so that you could potentially support `PATCH` operations in which you only send the fields that change.
...
##### The app with multiple models

With the multiple models in place, we can update the parts of the app that use them.
...
async def create_hero(hero: HeroCreate, session: SessionDep) -> Any:  # noqa: ANN401
...
##### Creating with `HeroCreate` and returning a `HeroPublic`

In the `POST /heroes` operation, we receive a `HeroCreate` and return a `HeroPublic`.

| NOTE: |
| :---- |
| The function declares a `response_model` because you're returning a different type from the one you want to give to your client. Alternatively, you could've used type hints and made your type tooling from ignore the fact that you're returning a different type from the one given in the type hints. |
...
##### Updating a hero with `HeroUpdate`

Now we can do a hero update using `PATCH /heroes/{hero_id}` without requiring receiving a full `Hero` object.
...
We get a dict with all the data sent by the client, but we require a `HeroUpdate` in the path operation, which enables FastAPI to validate the information sent.

Then, we use `hero_db.sqlmodel_update(hero_data)` to update `hero_db` with the data extracted from the information sent by the client.

### Structuring bigger applications

All the applications so far fit in a single file, but that's not the recommended way to create apps.

FastAPI provides a convenience tools to structure your app, while keeping all the flexibility.

#### An example

Let's assume you have a file structure like this:
...
Note that there are several `__init__.py` files, one in each directory or subdirectory. This enables importing modules in the following way. Effectively, the `__init__.py` identifies a directory as a *Python Package* (i.e., a collection of *Python Modules*)

```python
from app.routers import items
```

The relevant points about this structure are:
+ `app` contains your whole app. It has an empty `app/__init__.py` that identies the app as a package.
+ `app/main.py`, the main program and entry point of your app is a module within the `app` package.
+ `app/dependencies.py`, the module where you can keep your dependencies.
+ `app/routers` is a subpackage for your routers.
+ `app/routers/items.py` and `app/routers/users.py` are submodules within the `routers` subpackage to deal with specific business domain concerns (e.g., user and item management).
+ `app/internals` is a subpackage for your internal concerns.
+ `app/internals/admin.py` is a submodule within the `internals` subpackage.

#### APIRouter

Let's focus on the submodule `app/routers/users.py`. This should be the file dedicated to handle just users.

By using this approach, you can have your path operations related to users separated from the rest of code, to keep it organized, while still being part of the same FastAPI application (i.e., it's not a separate microservice for dealing with users).

FastAPI let's you do this using the `APIRouter`. You create an instance of the router and work with it in the same you'd do with a FastAPI app:
...
Note that the path operations are declared in exactly the same way as with the FastAPI class. All the same `parameter`, `responses`, `dependencies`, `tags`, etc. are available for `APIRouter` instances.

#### Dependencies

It's common to find yourself relying on dependencies that are used as cross-cutting concerns in several places of your app.

The example above suggests that you place them in an `app/dependencies.py` module.
...
async def get_token_header(x_token: Annotated[str, Header()]):
    if not x_token:
        raise HTTPException(status_code=400, detail="X-Token header must be present")
    return x_token

async def get_token_query(token: str):
    if not token:
        raise HTTPException(status_code=400, detail="token query parameter must be present")
    return token
```

#### More modules with APIRouter

When you have multiple *model families* in your application, the recommendation is to segregate each family into its own module.

For example, for managing items, you can use `app/routers/items.py` to host your path operations.

Besides, if your path operations share:
+ path prefixes
+ tags
+ extra responses
+ dependencies

You can define those things at the router level to avoid duplication in each of the path operations.
...
router = APIRouter(
    ...
    responses={404: {"description": "Not found"}} # this is for the /docs
)
...
@router.put("/{item_id}", tags=["custom"], responsed={403: {"description": "Operation forbidden"}})
...
Note that, as the path operation has to start with `/`, the prefix must not include a trailing `/`.

In the case above, the declared paths will end up being:
+ `/items/`: `/items` + `/`
+ `/items/{item_id}`: `/items` + `/{item_id}`.

Also, the the declared dependencies will apply to all the path operations. If you include additional dependencies in a path operation, they will be executed too.

The router dependencies will be executed first, then the dependencies in the path operation decorator, and finally, the parameter dependencies.

Note that for importing the dependencies, we've used:

```python
from ..dependencies import get_token_header
```

This is using relative imports to import from the parent package.

You can also use a single dot to import from the current package.
...
to import something from the parent of the parent's package.

#### The main `FastAPI`

In  `/app/main.py` is where you will instantiate your `FastAPI` class. Because the path operations, dependecies, etc. are hosted in their own modules, the main file will be quite simple.
...
app = FastAPI(dependencies=[Depends(get_token_query)])
...
You should start by importing your modules from the different subpackages. Because `users` and `items` are submodules of the same Python package `app`, you can use `.` to import them (using relative imports).

Note that you could have used absolute imports as well:
...
Note also that the code imports the entire modules `items` and `users` instead of the object that is really needed `router` in `items` module and `router` in `users` module to avoid name collisions. That way you don't need to use aliases.

Note that global dependencies can also be included when you instantiate your FastAPI app. Those will be combined with the ones you declare in your routers and and path operations (i.e., dependencies are additive).
...
Routers must be explicitly included in your FastAPI app using `FastAPI.include_router()` method:
...
You can use `include_router()` to customize the path operations of a router.

Let's imagine that you're given an `app/intenal/admin.py` with some operations your organization shares between several projects. Because it's a shared module, it doesn't contain a lot of customizations to make it subject of being used in many different applications:
...
In your FastAPI app defined in `app/main.py`, you'd like to customize the operations defined in `app/internal/admin.py` with things such as an appropriate prefix, apply the project's security scheme, include tags, and responses... You can do that with `app.include_router()`:
...
See how you've taken a given file and configure it with:
+ custom path prefix: "/admin"
+ clubbed their path operations under: "admin" group (docs)
+ configured their path operations with `get_token_header()` security
+ added response for 418 in the docs

And all of that without modifing the `app/inter/admin.py`. Effectively, `admin.py` could have been published as a separate package.

#### Including the same router multiple times with different prefix

You can use `app.include_router()` multiple times with the same router using different prefixes.

This could be useful when you're exposing the same API under different prefixes such as `/api/v1` and `/api/latest`.

#### Including an APIRouter in another

In the same way that you use `app.include_router()` to include a router in your FastAPI `app` instance, you can use `router.include_router()` to include an APIRouter in another APIRouter.

When doing so, you have to make your you do `router.include_router(subrouter)` before you do `app.include_router(router)` so that the operations defined in `subrouter` are also added to the application.

### Stream JSON Lines

You can use this functionality when you have a sequence of data that you'd like to send in a stream.

#### What is a stream?

Streaming data means your app will start sending data items to the client without waiting for the entire sequence of items to be ready.

That is, your app will send the first item (and the client will receive it), while you might still be producing the next item.

![Streaming](docs/009_streaming.png)

It could even be an infinite stream, where you keep sending indefinitely.

#### JSON Lines

In these cases, it's common to send "JSON Lines", which is a format where you send one JSON object per line, while the entire response is not a valid JSON object:
...
Note that there's no `[]` to wrap the object, or `,` between the items.

Theformat is commin in:
+ GenAI services
+ Logs
+ Telemetry data

#### Streaming JSON Lines with FastAPI

To stream JSON Lines from your FastAPI app, you just need to use `yield` instead of return in your path operation.
...
async def stream_items_sync() -> Iterable[Item]:
    for item in items:
        yield item
```
...
For example:
+ Email notifications that should be sent after performing an action.
+ Processing data asynchronously, instead of in realtime.

#### Using `BackgroundTasks`

You must first use `BackgroundTasks` and define a parameter in your path operation with a type declaration of type `BackgroundTasks`.

Then you can define a function/coroutine that will drive the background task.

| NOTE: |
| :---- |
| If your background task uses async code, use a coroutine. If it only uses sync code, use a regular function. |

Finally, you just need to use `BagroundTasks.add_task()` method that receives as arguments:
+ the task function that you want to run in the background
+ any sequence of arguments that should be passed to the task function (i.e., `*args`).
+ any keyword arguments that should be passed to the task function (i.e., `**kwargs`).
...
async def send_notification(email: str, background_tasks: BackgroundTasks):
    ...
    return {"message": "Notification has been sent in the background"}
```

#### Dependency injection

`BackgroundTasks` also works with FastAPI's dependency injection system. You can declare a parameter of type `BackgroundTasks` at multiple levels: in a path operation function (as above), in a dependency, in a sub-dependency, ...

FastAPI will know what to do in each case and will reuse the same object so that all background tasks will be merged together and run in the background afterwards.
...
def get_query(background_tasks: BackgroundTasks, q: str | None = None):
    if q:
        message = f"Found query in the request: {q}\n"
        background_tasks_add_task(write_log, message)
    return q
...
While it's possible to use `starlette.background.BackgroundTask` (note the missing **s**), you will have to create the object in your code and return a Starlette response.

#### Caveat

`BackgroundTasks` should be used to perform lightweight actions after you've sent your response.

If you intend to perform heavy background computation and you don't necessarily need it to be run by the same process, it's better to rely on frameworks like [Celery](https://docs.celeryq.dev/).

### Metadata and Docs URLs

#### Metadata for APIs

You can set the following fields that will be used in the OpenAPI spec, and the automatic API docs UIs:
...
| `version` | `str` | The version of the API (this is the verison of your own application). |
| `terms_of_service` | `str` | A URL to the *Terms of Service* for the API. |
| `contact` | `dict` | The contact information for the exposed API. It can contain the following fields: <ul><li>`name`: `str`</li><li>`url`: `str`, the URL poiting to the contact information</li><li>`email`: `str`, the email address of the contact person/organization.</li></ul> |
| `license_info` | `dict` | The license information for the exposed API. It can contain the following fields: <ul><li>`name`: `str`, the license name</li><li>`identifier`: `str`, the [SPDX](https://spdx.org/licenses/) license expression for the API.</li><li>`url`: `str`, url to the license used for the API.</li></ul> |
...
Since OpenAPI 3.1.0, you can also set the `license_info` with an identifier instead of a url:
...
+ `name`: required `str` with the same tag name used in the `tags` parameter in your path operations and `APIRouter`s.

+ `description`: a `str` with a short description for the tag. It can have Markdown and will be shown in the docs UI.

+ `externalDocs`: a `dict` describing external documentation with:
    + `description`: a `str` with a short description for the external docs.
    + `url`: required `str` with the URL for the external docs.
...
tags_metadata = [
    {
        "name": "users",
        "description": "Operations on users. The **login** logic is also here.",
    },
    {
        "name": "items",
        "description": "Manage items. So *fancy* they have their own docs.",
        "externalDocs": {
...
The order of each tag metadata dictionary also defined the order in which they are shown in the docs UI.

For example, `users` would go before `items` although it should be the contrary if using alphabetical order.

#### OpenAPI URL

By default, OpenAPI schema is served at `/openapi.json`. This can be customized using the `openapi_url` parameter:

```python
app = FastAPI(openapi_url="/api/v1/openapi.json")
```

You can disable exposing the OpenAPI schema by setting `openapi_url=None`. That will also disable the document user interfaces that use it.

| NOTE: |
| :---- |
| Disabling the OpenAPI schema generation and docs UIs is a sensible option for production deployments. |

#### Docs URLs

You can configure the two documentation user interfaces included by default:

+ Swagger UI: served at `/docs` by default.
    + You can customize the path using `docs_url`.
    + You can disable it by setting `docs_url=None`

+ ReDoc: served at `/redoc` by default.
    + You can customize the path using `redoc_url`.
...
| NOTE: |
| :---- |
| Disabling the docs UIs is a sensible option for production deployments. |

### Static files

You can serve files automatically from a directory using `StaticFiles`.
...
Note that after importing `StaticFiles`, you need to *mount* a `StaticFiles` instance in a specific path.

Mounting means adding a complete independent application in a specific path, that then takes care of handling all the subpaths.

This is different from using an APIRouter, as a mounted application is completely independent. For example, you won't find anything from the mounted application in the OpenAPI schema or docs.
...
+ `/static`: refers to the subpath where this subapplication will be mounted on. In the example, by using "/static" you declare that any path starting with `/static` will be handled by it.
+ `StaticFiles(directory="static")`: refers to the name of the directory that contains your static files.
+ `name=static`: gives the mount a name that can be used internally by FastAPI.

### Testing

Testing in FastAPI can be done using [pytest](https://docs.pytest.org/).

#### Using `TestClient`

The following snippet illustrates how you can use `TestClient` to interact with your FastAPI application in your tests.
...
Note that the testing functions are normal functions and not coroutines. Also, notice that you don't need to await then using `client.get("/")`.

#### Separating tests

In real applications, your tests will be hosted in different files from your app source files.

For example, if following the bigger application structure, you had:
...
So, the `main.py` is where you'd have your path operations:

```python
"""main.py in the app package."""
...
Then, you could have a `test_main.py` file with your tests, living in the same Python package as `main.py`:
...
When using this approach, because the test file is in the same package as module you're testing, you will be able to use relative imports:
...
#### Testing: an extended example

Let's consider a slightly more complex `main.py`. This will let you understand more details about testing the different parts of a FastAPI app.
...
fake_secret_token = "***" # some secret token
...
@app.get("/items/{item_id}", responde_model=Item)
...
In the example, we have a couple of path operations that may return wrror responses, both requiring the presence of an "X-Token" header with a particular value.

The corresponding `test_main.py` file would be:
...
def test_read_item_bad_token():
    response = client.get("/items/foo", headers={"X-Token": "(redacted)"})
    assert response.status_code == 400
    assert response.json == {"detail": "Invalid X-Token header"}

def test_read_item_nonexistent_item():
    response = client.get("/items/bax", headers={"X-Token": "***"})
...
def test_create_item():
    response = client.post(
        "/items/",
        headers={"X-Token": "***"},
        json={"id": "foobar", "title": "Foo Bar", "description": "The Foo Bar Fighters"},
    )
    assert response.status_code == 200
    assert response.json() == {
        "id": "foobar",
        "title": "Foo Bar",
        "description": "The Foo Bar Fighters",
    }
...
def test_create_item_bad_token():
    response = client.post(
        "/items/",
        headers={"X-Token": "xyz"},
        json={"id": "baz", "title": "Baz", "description": "The ugly and the baz"},
    )
    assert response.status_code == 400
    assert response.json() == {"detail": "Invalid X-Token header"}
...
def test_create_item_existing_item():
    response = client.post(
        "/items/",
        headers={"X-Token": "xyz"},
        json={"id": "foo", "title": "Foo", "description": "The Foo, the bar, and the bazz"},
    )
    assert response.status_code == 409
    assert response.json() == {"detail": "Item already exists"}
```

`TestClient` design is based on [`httpx`](https://www.python-httpx.org/), which in turn is based on `requests` design. Therefore, you can google how to do the same in your tests for:
...
With the testing in place, you just need to install and run pytest.


### Debugging

One of the easiest ways to connect your debugger to your FastAPI application is to call directly `uvicorn` in your main file.
...
By doing so, you will be able to use the "Python: Current File (Integrated Terminal)".

#### VSCode debugging configuration

Alternatively, you can use the following VSCode configuration in your project:
...
```json
{
  ...
  "configurations": [
    {
      // the console on which it runs need to have the venv activated
      "name": "FastAPI dev port 5050",
      "type": "debugpy",
      "request": "launch",
      "module": "fastapi",
      "args": [
