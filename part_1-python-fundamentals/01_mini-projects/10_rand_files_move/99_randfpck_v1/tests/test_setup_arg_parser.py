...
from randfpck.main import setup_arg_parser
...
class TestArgParserSetup(unittest.TestCase):
    ...
    def test_description_not_empty(self):
        ...
        self.assertIsNotNone(
            parser.description,
            "expected parser.description to be not None, but was None",
        )
        self.assertNotEqual(
            parser.description,
            "",
            "expected parser.description not to be empty, but was ''",
        )
    ...
    def test_parser_unhappy_path(self):
        ...
        test_cases = {
            ...
            "Source and dst": TestCase(
                cli_args=["-s", "src_folder/", "-d", "dst_folder/"]
            ),
            "Source and glob": TestCase(
                cli_args=["-s", "src_folder/", "-g", "glob.*"]
            ),
            "Source and num files": TestCase(
                cli_args=["-s", "src_folder/", "-n", "5"]
            ),
        ...
        for test_case, test_data in test_cases.items():
            ...
            with self.assertRaises(
                SystemExit,
                msg=f"{test_case}: expected exception but nothing was raised",
            ) as ex:
    ...
    def test_parser_happy_path_req_args(self):
        ...
        try:
            ...
            self.assertEqual(
                args.src,
                "src_folder/",
                f"expected 'src_folder/' but got {args.src!r}",
            )
            self.assertEqual(
                args.dst,
                "dst_folder/",
                f"expected 'dst_folder/' but got {args.dst!r}",
            )
            self.assertListEqual(
                args.glob,
                ["glob.*"],
                f"expected ['glob.*'] but got {args.glob}",
            )
            self.assertEqual(
                args.num_files, 5, f"expected 5 but got {args.num_files}"
            )

    def test_parser_happy_path_with_opt_args(self):
        """Happy path for arg parsing when providing required and optional
        args.
        """
        parser = setup_arg_parser()
        ...
        try:
            ...
            self.assertEqual(
                args.used,
                "used_folder/",
                f"expected 'used_folder/' but got {args.used!r}",
            )
            self.assertListEqual(
                args.glob,
                ["glob.*"],
    ...
    def test_parser_happy_path_with_multipl_glob(self):
        ...
        try:
            ...
            self.assertListEqual(
                ...
                (
                    f'expected ["glob1.*", "glob2", "*glob3*.txt"] '
                    f"but got {args.glob}"
                ),
            )
            self.assertEqual(
