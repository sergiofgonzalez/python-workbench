...
def process_file(request: CustomRequestHandler) -> None:
    ...
    with Path(dest_filename).open("wb") as f:
        while chunk := request.rfile.read(min(remaining_bytes, 256)):
            f.write(chunk)
            remaining_bytes -= min(remaining_bytes, 256)
    logger.info("{} saved", filename)


def run_server(port: int | None = 8000) -> None:
    """Run an HTTP server that uncompresses and saves a file received in the request."""
    server_address = ("", port)
