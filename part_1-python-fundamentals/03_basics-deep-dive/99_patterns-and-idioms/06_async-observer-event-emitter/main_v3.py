...
class EventEmitter:
    ...
    async def gather_listeners(self) -> None:
        """Schedules the execution of the waiters in the event loop."""
        waiter_tasks = [
            waiter_task
            for waiters in self.cb_registry.values()
...
async def main() -> None:
    ...
    # because the never evt is never fired, the program runs indefinitely
    await event_emitter.gather_listeners()
