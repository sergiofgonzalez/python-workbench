...
def show_file_selector():
    ...
    if uploaded_pdf is not None:
        previous_pdf_info = read_app_state(SessionKeys.PDF_FILE_INFO)
        logger.debug(
            "PDF file selected: '{}', previous file: '{}'",
            uploaded_pdf.name,
            previous_pdf_info.friendly_name if previous_pdf_info else None,
        )
        pdf_bytes = uploaded_pdf.read()
        hash = hashlib.md5(pdf_bytes).hexdigest()
        if previous_pdf_info and hash == previous_pdf_info.hash:
            logger.debug(
                "PDF file '{}' has not changed: no need to reprocess",
                uploaded_pdf.name,
            )
            return
        with tempfile.NamedTemporaryFile(delete=False) as f:
            ...
            uploaded_pdf_info = PdfFileInfo(
                friendly_name=uploaded_pdf.name, temp_path=f.name, hash=hash
            )
