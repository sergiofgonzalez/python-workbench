...
logger.info("Using vec2d.graph v0.1.0")
...
class Polygon(Figure2D):
    ...
    def render(self) -> None:
        ...
        if self.fill:
            ...
            plt.gca().fill(xs, ys, self.fill.value if hasattr(self.fill, "value") else self.fill, alpha=self.alpha)


class Arrow(Figure2D):
    """Represents an arrow on the 2D plane, defined by its tip and tail specified
    by their coordinates (x, y). If tail is not provided the arrow is assumed to
    have its tail in the origin of coordinates (0, 0). The color argument sets
    ...
    *objects: Sequence[Figure2D],
    ...
    Args:
        objects (Sequence[Figure2D]): the list of figures to be displayed in the
            plot.
        origin (bool, optional): whether to show or hide the origin of coordinates.
            Default is to show the origin (True). You can pass None or False if
            you don't want the origin to be displayed.
        ...
        grid (tuple[int | float, int | float], optional): sets the frequency for the ticks in the plot
            for x- and y- axes. Default is (1, 1) meaning that you will find a tick per unit
            in both x- and y- axes. You can pass None or False if you don't want
