...
The library exposes classes for the figures, an enumeration for the common colors,
and a function draw() to render the figures.
"""
...
logger.info("Using vec2d v0.1.0")
...
class Figure2D(ABC):
    ...
    def extract_vectors(self) -> tuple[int | float, int | float]:
        """Generator function that returns the vectors (points) that define the figure.
    ...
    def render(self) -> None:
        """Performs the necessary actions on Matplotlib to render the corresponding
        figure on screen.
        """
...
class Points(Figure2D):
    ...
    def extract_vectors(self) -> tuple[int | float, int | float]:
        for v in self.vectors:  # pylint: disable=invalid-name
            yield v

    def render(self) -> None:
        xs = [v[0] for v in self.vectors]  # pylint: disable=invalid-name
        ys = [v[1] for v in self.vectors]  # pylint: disable=invalid-name
        plt.scatter(xs, ys, color=self.color.value)
...
class Segment(Figure2D):
    ...
    def render(self) -> None:
        ...
        plt.plot([x1, x2], [y1, y2], color=self.color.value)


class Polygon(Figure2D):
    """Represents a polygon on the 2D plane, defined by its vertices using their
    (x,y) coordinates. You can specify the color of the lines delimiting the polygon
    using color. By default, the lines will be drawn in blue. If a color is specified
    for the fill parameter, the polygon will be filled. By default, the polygon will
    not be filled. The alpha parameter can be used to established the alpha blending
    parameter. By default, the alpha blending is set to a 0.4 value.
    """
    ...
    def render(self) -> None:
        for i in range(0, len(self.vertices)):  # pylint: disable=consider-using-enumerate
            x1, y1 = self.vertices[i]  # pylint: disable=invalid-name
            x2, y2 = self.vertices[(i + 1) % len(self.vertices)]  # pylint: disable=invalid-name
            plt.plot([x1, x2], [y1, y2], color=self.color.value)
        if self.fill:
            xs = [v[0] for v in self.vertices]  # pylint: disable=invalid-name
            ys = [v[1] for v in self.vertices]  # pylint: disable=invalid-name
            plt.gca().fill(xs, ys, self.fill.value, alpha=self.alpha)


class Arrow(Figure2D):
    """Represents an arrow on the 2D plane, defined by its tip and tail specified
    by their coordinates (x, y). If tail is not provided the arrow is is assumed
    to have its tail in the origin of coordinates (0, 0). The color argument sets
    the color of the segment and head of the arrow. By default, arrows are drawn in
    red.
    """
