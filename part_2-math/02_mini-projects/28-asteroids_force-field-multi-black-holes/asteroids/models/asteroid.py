...
def gravitational_field(sources, x, y):
    ...
    """
    fields = [
        scale(-source.gravity, (x - source.x, y - source.y))
        for source in sources
    ]
    if len(fields) >= 2:
        return add_mult(*fields)
    else:
        relative_position = (x - sources[0].x, y - sources[0].y)
        return scale(-sources[0].gravity, relative_position)
